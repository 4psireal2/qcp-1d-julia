...
open("cp_lindblad.txt", "w") do file
    for row in 1:size(lindbladMatrix, 1)
        for col in 1:size(lindbladMatrix, 2)
            @printf(file, "(%.3f + %.3fim)", real(lindbladMatrix[row, col]), imag(lindbladMatrix[row, col]))
...
# @tensor darkState[-1 -2 -3 -4 -5 -6] :=  boundaryL[1] * darkState[1][1,-1,-2,2] * darkState[2][2,-3,-4,3] * darkState[3][3,-5,-6,4] * boundaryR[4];
...
# randomMPS[1] = TensorMap(randImMat, ComplexSpace(1) ⊗ ComplexSpace(d) ⊗ ComplexSpace(d)',  ComplexSpace(1));
# for i = 2 : (N-1)
#     randImMat = randn(rng, ComplexF32, (2, 2))
#     randomMPS[i] = TensorMap(randImMat, ComplexSpace(1) ⊗ ComplexSpace(d) ⊗ ComplexSpace(d)',  ComplexSpace(1));
# end
# randImMat = randn(rng, ComplexF32, (2, 2))
# randomMPS[N] = TensorMap(randn, ComplexSpace(1) ⊗ ComplexSpace(d) ⊗ ComplexSpace(d)', ComplexSpace(1));

# boundaryL = TensorMap(ones, one(ComplexSpace()), ComplexSpace(1))
# boundaryR = TensorMap(ones, ComplexSpace(1), one(ComplexSpace()))

# @tensor randomMPSmat[-1 -2 -3; -4 -5 -6] :=  boundaryL[1] * randomMPS[1][1,-1,-4,2] * randomMPS[2][2,-2,-5,3] * randomMPS[3][3,-3,-6,4] * boundaryR[4];
# randomMPSmat = reshape(convert(Array, randomMPSmat), (8,8));

# randomMPS_dag = Vector{TensorMap}(undef, N);
# for i = 1 : N
#     randomMPS_dag[i] = TensorMap(conj(permutedims(convert(Array, randomMPS[i]), (1,3,2,4))), codomain(randomMPS[i]), domain(randomMPS[i])); 
# end

# boundaryL = TensorMap(ones, one(ComplexSpace()), ComplexSpace(1))
# boundaryR = TensorMap(ones, ComplexSpace(1), one(ComplexSpace()))

# @tensor randomMPSdag_mat[-1 -2 -3; -4 -5 -6] :=  boundaryL[1] * randomMPS_dag[1][1,-1,-4,2] * randomMPS_dag[2][2,-2,-5,3] * randomMPS_dag[3][3,-3,-6,4] * boundaryR[4];
...
# boundaryL = TensorMap(ones, one(ComplexSpace()), ComplexSpace(1))
# boundaryR = TensorMap(ones, ComplexSpace(1), one(ComplexSpace()))
# @tensor hermit_mat[-1 -2 -3; -4 -5 -6] :=  boundaryL[1] * hermit_MPS[1][1,-1,-4,2] * hermit_MPS[2][2,-2,-5,3] * hermit_MPS[3][3,-3,-6,4] * boundaryR[4];
