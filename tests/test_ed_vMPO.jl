...
@tensor rho[-1 -2 -3; -4 -5 -6] :=
    boundaryL[8, 1] *
    XInit[1][1, 2, -4, 3] *
    conj(XInit[1][8, 2, -1, 9]) *
    XInit[2][3, 4, -5, 5] *
    conj(XInit[2][9, 4, -2, 10]) *
    XInit[3][5, 6, -6, 7] *
    conj(XInit[3][10, 6, -3, 11]) *
    boundaryR[7, 11]
...
for i in 1:N
    ...
    boundaryL = TensorMap(ones, one(ComplexSpace()), ComplexSpace(1))
    boundaryR = TensorMap(ones, ComplexSpace(1), one(ComplexSpace()))
    @tensor numberOpSite[-1 -2 -3; -4 -5 -6] :=
        boundaryL[1] *
        numberOpSite[1][1, -1, -4, 2] *
        numberOpSite[2][2, -2, -5, 3] *
        numberOpSite[3][3, -3, -6, 4] *
        boundaryR[4]
    numberOps[i] = reshape(
        permutedims(convert(Array, numberOpSite), (1, 4, 2, 5, 3, 6)), (64, 1)
    )
end
...
plot!(
    ...
    avOccTEBD_t;
    ls=:dash,
    label="TEBD+LPTN; CHI=$(BONDDIM), KRAUSDIM=$(KRAUSDIM)",
)
