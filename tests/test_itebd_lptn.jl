...
Go = TensorMap(tensorBase1, ComplexSpace(bondDim) ⊗ ComplexSpace(krausDim), ComplexSpace(d) ⊗ ComplexSpace(bondDim));
Ge = TensorMap(tensorBase0, ComplexSpace(bondDim) ⊗ ComplexSpace(krausDim), ComplexSpace(d) ⊗ ComplexSpace(bondDim));
...
@tensor leftEnv[-2; -1] := Go[1, 2, 3, -1] * Le[4, 1] * conj(Le[4, 5]) * conj(Go[5, 2, 3, -2]);
@show leftEnv
...
@tensor rightEnv[-1; -2] := Ge[-1, 1, 2, 3] * Le[3, 4] * conj(Le[5, 4]) * conj(Ge[-2, 1, 2, 5]);
@show rightEnv
...
Go, Ge, Lo, Le = iTEBD_for_LPTN!(Go, Ge, Lo, Le, hamDyn, hamDyn, dissDyn, bondDimTrunc, krausDimTrunc);
