...
@tensor rightEnv[-1 -4; -2 -3] := bondTensor[-1, 1, 2, 3] * conj(bondTensor[-2, 1, 2, 4]) * Le[3, -3] * conj(Le[4, -4]);
@tensor transferR[-1; -2] := bondTensor[-1, 1, 2, 3] * conj(bondTensor[-2, 1, 2, 5]) * Le[3, 4] * conj(Le[5, 4]);

@tensor leftEnv[-1 -4; -2 -3] := Le[-1, 1] * bondTensor[1, 2, 3, -3] * conj(Le[-2, 4]) * conj(bondTensor[4, 2, 3, -4]);
@tensor transferL[-2; -1] := Le[1, 2] * bondTensor[2, 3, 4, -1] * conj(Le[1, 5]) * conj(bondTensor[5, 3, 4, -2]);
