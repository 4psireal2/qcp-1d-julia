...
function initializeRandomMPS(N, d::Int64=2; bonddim::Int64=1)::Vector{TensorMap}
    ...
    randomMPS[1] = TensorMap(
        ones, ComplexSpace(1) ⊗ ComplexSpace(d) ⊗ ComplexSpace(d)', ComplexSpace(bonddim)
    )
    for i in 2:(N - 1)
        randomMPS[i] = TensorMap(
            ones,
            ComplexSpace(bonddim) ⊗ ComplexSpace(d) ⊗ ComplexSpace(d)',
            ComplexSpace(bonddim),
        )
    end
    randomMPS[N] = TensorMap(
        ones, ComplexSpace(bonddim) ⊗ ComplexSpace(d) ⊗ ComplexSpace(d)', ComplexSpace(1)
    )
...
function initializeBasisMPS(N::Int64, basis::Vector; d::Int64=2)::Vector{TensorMap}
    ...
    for i in 1:N
        ...
        tensor[1, :, :, 1] = basis[i]
        mps[i] = TensorMap(
            tensor, ComplexSpace(1) ⊗ ComplexSpace(d) ⊗ ComplexSpace(d)', ComplexSpace(1)
        )
    end
...
function orthogonalizeMPS(mps, orthoCenter::Int=1)::Vector{TensorMap}
    ...
    for i in 1:1:(orthoCenter - 1)
        ...
        orthoMPS[i + 0] = Q
        orthoMPS[i + 1] = permute(
            R * permute(orthoMPS[i + 1], (1,), (2, 3, 4)), (1, 2, 3), (4,)
        )
    end
    ...
    for i in N:-1:(orthoCenter + 1)
        ...
        orthoMPS[i - 1] = permute(
            permute(orthoMPS[i - 1], (1, 2, 3), (4,)) * L, (1, 2, 3), (4,)
        )
...
function multiplyMPOMPO(mpo1::Vector{TensorMap}, mpo2::Vector{TensorMap})::Vector{TensorMap}
    ...
    for i in 1:N
        @tensor resultMPO[i][-1 -2 -3; -4 -5 -6] :=
            mpo1[i][1 2 3; -4 -5 4] *
            mpo2[i][5 -2 -3; 2 3 6] *
            fusers[i][-1; 1 5] *
...
function multiplyMPOMPS(mpo::Vector{TensorMap}, mps::Vector{TensorMap})::Vector{TensorMap}
    ...
    fusers = PeriodicArray(
        map(zip(mps, mpo)) do (mpo, mps)
            return isometry(
                fuse(space(mpo, 1), space(mps, 1)), space(mpo, 1) * space(mps, 1)
            )
        end,
    )
    ...
    for i in 1:N
        @tensor resultMPS[i][-1 -2 -3; -4 -5 -6] :=
            mpo[i][1 2 3; -4 -5 4] *
            mps[i][5 -2 -3; 2 3 6] *
            fusers[i][-1; 1 5] *
            conj(fusers[i + 1][-6; 4 6])
...
function addMPSMPS(mpoA, mpoB)::Vector{TensorMap}
    ...
    isoRA =
        isometry(
            space(mpoA[idxMPO], 4)' ⊕ space(mpoB[idxMPO], 4)', space(mpoA[idxMPO], 4)'
        )'
    isoRB = rightnull(isoRA)
    @tensor newTensor[-1 -2 -3; -4] :=
        mpoA[idxMPO][-1, -2, -3, 4] * isoRA[4, -4] +
        mpoB[idxMPO][-1, -2, -3, 4] * isoRB[4, -4]
    mpoC[idxMPO] = newTensor

    # bulk tensors
    for idxMPO in 2:(N - 1)
        isoLA = isometry(
            space(mpoA[idxMPO], 1) ⊕ space(mpoB[idxMPO], 1), space(mpoA[idxMPO], 1)
        )
        ...
        isoRB = rightnull(isoRA)
        @tensor newTensor[-1 -2 -3; -4] :=
            isoLA[-1, 1] * mpoA[idxMPO][1, -2, -3, 4] * isoRA[4, -4] +
            isoLB[-1, 1] * mpoB[idxMPO][1, -2, -3, 4] * isoRB[4, -4]
        mpoC[idxMPO] = newTensor
    end
    ...
    isoLB = leftnull(isoLA)
    @tensor newTensor[-1 -2 -3; -4] :=
        isoLA[-1 1] * mpoA[idxMPO][1 -2 -3 -4] + isoLB[-1 1] * mpoB[idxMPO][1 -2 -3 -4]
    mpoC[idxMPO] = newTensor
    return mpoC
end
...
function computeRhoDag(mps)
    ...
    for i in 1:length(mps)
        mpsDag[i] = TensorMap(
            conj(permutedims(convert(Array, mps[i]), (1, 3, 2, 4))),
            codomain(mps[i]),
            domain(mps[i]),
        )
    end
