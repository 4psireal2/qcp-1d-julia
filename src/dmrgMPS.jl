...
function initializeRandomMPS(N; d::Int64=2, bondDim::Int64=1)
    ...
    randomMPS[1] = TensorMap(
        randn, ComplexSpace(1) ⊗ ComplexSpace(d), ComplexSpace(bondDim)
    )
    for idxMPO in 2:(N - 1)
        randomMPS[idxMPO] = TensorMap(
            randn, ComplexSpace(bondDim) ⊗ ComplexSpace(d), ComplexSpace(bondDim)
        )
    end
    randomMPS[N] = TensorMap(
        randn, ComplexSpace(bondDim) ⊗ ComplexSpace(d), ComplexSpace(1)
    )
...
function orthogonalizeMPS!(finiteMPS::Vector{<:AbstractTensorMap}, orthCenter::Int)
    ...
    for siteIdx in 1:+1:(orthCenter - 1)
        ...
        finiteMPS[siteIdx + 1] = permute(
            R * permute(finiteMPS[siteIdx + 1], (1,), (2, 3)), (1, 2), (3,)
        )
    end

    # bring sites orthCenter + 1 to N into right-canonical form
    for siteIdx in N:-1:(orthCenter + 1)
        (L, Q) = rightorth(finiteMPS[siteIdx], (1,), (2, 3); alg=LQpos())
        finiteMPS[siteIdx - 1] = permute(
            permute(finiteMPS[siteIdx - 1], (1, 2), (3,)) * L, (1, 2), (3,)
        )
        finiteMPS[siteIdx - 0] = permute(Q, (1, 2), (3,))
    end
    return finiteMPS
...
function applyH2(X, EL, mpo1, mpo2, ER)
    @tensor X[-1 -2; -3 -4] :=
        EL[-1, 2, 1] * X[2, 3, 5, 6] * mpo1[1, -2, 3, 4] * mpo2[4, -3, 5, 7] * ER[6, 7, -4]
    return X
end
...
function initializeMPOEnvironments(
    finiteMPS::Vector{<:AbstractTensorMap},
    finiteMPO::Vector{<:AbstractTensorMap};
    centerPos::Int64=1,
)
    ...
    mpoEnvL[1] = TensorMap(
        ones, space(finiteMPS[1], 1), space(finiteMPS[1], 1) ⊗ space(finiteMPO[1], 1)
    )
    mpoEnvR[N] = TensorMap(
        ones, space(finiteMPS[N], 3)' ⊗ space(finiteMPO[N], 4)', space(finiteMPS[N], 3)'
    )

    # compute mpoEnvL up to (centerPos - 1)
    for siteIdx in 1:+1:(centerPos - 1)
        mpoEnvL[siteIdx + 1] = update_MPOEnvL(
            mpoEnvL[siteIdx], finiteMPS[siteIdx], finiteMPO[siteIdx], finiteMPS[siteIdx]
        )
    end
    ...
    for siteIdx in N:-1:(centerPos + 1)
        mpoEnvR[siteIdx - 1] = update_MPOEnvR(
            mpoEnvR[siteIdx], finiteMPS[siteIdx], finiteMPO[siteIdx], finiteMPS[siteIdx]
        )
    end
    ...
    while runOptimizationDMRG
        ...
        for siteIdx in 1:+1:(N - 1)
            ...
            eigenVal, eigenVec = eigsolve(
                theta, 1, :SR, KrylovKit.Lanczos(; tol=eigsTol, maxiter=maxIterations)
            ) do x
                applyH2(
                    x,
                    mpoEnvL[siteIdx],
...
function computeExpValMPO(
    finiteMPS::Vector{<:AbstractTensorMap}, finiteMPO::Vector{<:AbstractTensorMap}
)
    ...
    for siteIdx in 1:+1:N
        @tensor boundaryL[-1; -2 -3] :=
            boundaryL[1, 5, 3] *
            finiteMPS[siteIdx][5, 4, -2] *
            finiteMPO[siteIdx][3, 2, 4, -3] *
...
function computeNorm(mps)
    ...
    for i in 1:N
        @tensor boundaryL[-2; -1] :=
            mps[i][1, 2, -1] * conj(mps[i][3, 2, -2]) * boundaryL[3, 1]
    end
...
function computeCorr2!(mps, mpo, index1, index2)
    ...
    for i in index1:index2
        ...
        if i == index1 || i == index2
            @tensor boundaryL[-2; -1] :=
                mps[i][1, 2, -1] * mpo[4, 2] * conj(mps[i][3, 4, -2]) * boundaryL[3, 1]
        else
            @tensor boundaryL[-2; -1] :=
                mps[i][1, 2, -1] * conj(mps[i][3, 2, -2]) * boundaryL[3, 1]
        end
