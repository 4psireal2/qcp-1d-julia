...
function update_MPSEnvL(mpsEnvL, mpsTensorK, mpsTensorB)
    @tensor newEL[-1; -2] :=
        mpsEnvL[1, 3] * mpsTensorK[3, 2, 4, -2] * conj(mpsTensorB[1, 2, 4, -1])
    return newEL
end

function update_MPSEnvR(mpsEnvR, mpsTensorK, mpsTensorB)
    @tensor newER[-1; -2] :=
        mpsTensorK[-1, 2, 4, 1] * conj(mpsTensorB[-2, 2, 4, 3]) * mpsEnvR[1, 3]
    return newER
end

function initializePROEnvironments(
    ketMPS::Vector{TensorMap}, orthStates::Vector{Vector{TensorMap}}; centerPos::Int64=1
)
    ...
    for orthIdx in eachindex(orthStates)
        ...
        for siteIdx in 1:+1:(centerPos - 1)
            projEnvL[siteIdx + 1] = update_MPSEnvL(
                projEnvL[siteIdx], ketMPS[siteIdx], braMPS[siteIdx]
            )
        end
        ...
        for siteIdx in N:-1:2
            projEnvR[siteIdx - 1] = update_MPSEnvR(
                projEnvR[siteIdx], ketMPS[siteIdx], braMPS[siteIdx]
            )
        end
...
function applyH1_X(X, siteIdx, EL, MPO, ER, PLs, Y, PRs, penaltyWeights::Vector{Float64})
    ...
    for orthIdx in eachindex(Y)
        waveFunctionOverlap = @tensor PLs[orthIdx][siteIdx][1, 2] *
            ...
            PRs[orthIdx][siteIdx][5, 6] #XXX:  PRs[orthIdx][siteIdx + 1]?
    ...
    for orthIdx in eachindex(Y)
        @tensor XO[-1 -2 -3; -4] :=
            conj(PLs[orthIdx][siteIdx][1, -1]) *
            Y[orthIdx][1, -2, -3, 4] *
            conj(PRs[orthIdx][siteIdx][-4, 4]) #XXX:  conj(PRs[orthIdx][siteIdx + 1][-4, 4])?
...
function find_excitedstate!(
    finiteMPS::Vector{TensorMap},
    finiteMPO::Vector{TensorMap},
    previousMPS::Vector{<:Vector{TensorMap}},
    alg::DMRG1_params,
)
    N = length(finiteMPS)
    ...
    while runOptimizationDMRG
        ...
        for siteIdx in 1:+1:N
            ...
            for orthMPO in eachindex(previousMPS)
            ...
            if siteIdx < N
                ...
                finiteMPS[siteIdx + 1] = permute(
                    R * permute(finiteMPS[siteIdx + 1], (1,), (2, 3, 4)), (1, 2, 3), (4,)
                )

                # shift orthogonality center of previousMPS to the left
                for orthIdx in eachindex(previousMPS)
                    (Q, R) = leftorth(
                        previousMPS[orthIdx][siteIdx], (1, 2, 3), (4,); alg=QRpos()
                    )
                    previousMPS[orthIdx][siteIdx + 0] = permute(Q, (1, 2, 3), (4,))
                    previousMPS[orthIdx][siteIdx + 1] = permute(
                        R * permute(previousMPS[orthIdx][siteIdx + 1], (1,), (2, 3, 4)),
                        (1, 2, 3),
                        (4,),
                    )
                    ...
                    projEnvsL[orthIdx][siteIdx + 1] = update_MPSEnvL(
                        ...
                        finiteMPS[siteIdx],
                        previousMPS[orthIdx][siteIdx],
                    )
                end

                # update mpoEnvL
                mpoEnvL[siteIdx + 1] = updateMPOEnvL(
                    mpoEnvL[siteIdx],
                    finiteMPS[siteIdx],
                    finiteMPO[siteIdx],
                    finiteMPS[siteIdx],
        ...
        for siteIdx in N:-1:1
            ...
            if siteIdx > 1
                ...
                finiteMPS[siteIdx] = permute(Q, (1, 2, 3), (4,))
                finiteMPS[siteIdx - 1] = permute(
                    finiteMPS[siteIdx - 1] * L, (1, 2, 3), (4,)
                )

                # shift orthogonality center of previousMPS to the right
                for orthIdx in eachindex(previousMPS)
                    (L, Q) = rightorth(
                        previousMPS[orthIdx][siteIdx], (1,), (2, 3, 4); alg=LQpos()
                    )
                    previousMPS[orthIdx][siteIdx - 1] = permute(
                        permute(previousMPS[orthIdx][siteIdx - 1], (1, 2, 3), (4,)) * L,
                        (1, 2, 3),
                        (4,),
                    )
                    previousMPS[orthIdx][siteIdx + 0] = permute(Q, (1, 2, 3), (4,))

                    projEnvsR[orthIdx][siteIdx - 1] = update_MPSEnvR(
                        projEnvsR[orthIdx][siteIdx],
                        finiteMPS[siteIdx],
                        previousMPS[orthIdx][siteIdx],
                    )
                end
                ...
                # update mpoEnvR
                mpoEnvR[siteIdx - 1] = updateMPOEnvR(
                    mpoEnvR[siteIdx],
                    finiteMPS[siteIdx],
                    finiteMPO[siteIdx],
                    finiteMPS[siteIdx],
