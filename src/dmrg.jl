...
function updateMPOEnvL(
    mpoEnvL::TensorMap, mpsT::TensorMap, mpo::TensorMap, mpsB::TensorMap
)::TensorMap
...
function updateMPOEnvR(
    mpoEnvR::TensorMap, mpsT::TensorMap, mpo::TensorMap, mpsB::TensorMap
)::TensorMap
...
function applyH1(X::TensorMap, EnvL::TensorMap, mpo::TensorMap, EnvR::TensorMap)::TensorMap
    @tensor X[-1 -2 -3; -4] :=
        EnvL[-1, 2, 1] * X[2, 3, 4, 5] * mpo[1, -2, -3, 3, 4, 6] * EnvR[5, 6, -4]
    return X
end

function applyH2(
    X::TensorMap, EnvL::TensorMap, mpo1::TensorMap, mpo2::TensorMap, EnvR::TensorMap
)::TensorMap
...
function initializeMPOEnvs(
    mps::Vector{TensorMap}, mpo::Vector{TensorMap}; centerPos::Int64=1
)
    """
...
function DMRG1(
    mps::Vector{TensorMap},
    mpo::Vector{TensorMap};
    ...
    while runOptimizationDMRG
        ...
        for i in 1:+1:N
            ...
            _, eigenVec = eigsolve(
                mps[i], 1, :SR, KrylovKit.Lanczos(; tol=eigsTol, maxiter=maxIterations)
            ) do x
            ...
            if i < N
                ...
                mps[i + 1] = permute(
                    R * permute(mps[i + 1], (1,), (2, 3, 4)), (1, 2, 3), (4,)
                )
...
function DMRG2(
    ...
    while runOptimizationDMRG
        ...
        for i in 1:1:(N - 1)
            ...
            # optimize wave function to get newTensor
            _, eigenVec = eigsolve(
                bondTensor, 1, :SR, Arnoldi(; tol=eigsTol, maxiter=maxIterations)
            ) do x # 1 eigenVal
